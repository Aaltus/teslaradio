...
public class SensorAccessJMEActivity extends AndroidHarness {
	...
	private int mDesiredCameraPreviewWidth = 1280;
    ...
    private SensorEventListener sensorListener = new SensorEventListener() {
		...
		public void onSensorChanged(SensorEvent event) {
    		...
    		case Sensor.TYPE_LINEAR_ACCELERATION:
               // System.out.println("TYPE_LINEAR_ACCELERATION");
    			//do something
    			break;
...
//                float[] rotationVector1= {event.values[0],event.values[1], event.values[2]};
//                float[] quaternion1 = {0.f,0.f,0.f,0.f};
//                sensorManager.getQuaternionFromVector(quaternion1,rotationVector1);
//                float qw1 = quaternion1[0]; float qx1 = quaternion1[1];
//                float qy1 = quaternion1[2]; float qz1 = quaternion1[3];
//                double headingQ1 = Math.atan2(2*qy1*qw1-2*qx1*qz1 , 1 - 2*qy1*qy1 - 2*qz1*qz1);
//                double pitchQ1 = Math.asin(2*qx1*qy1 + 2*qz1*qw1);
//                double rollQ1 = Math.atan2(2*qx1*qw1-2*qy1*qz1 , 1 - 2*qx1*qx1 - 2*qz1*qz1);
//                if ((com.ar4android.sensorAccessJME.SensorAccessJME) app != null) {
//                    ((com.ar4android.sensorAccessJME.SensorAccessJME) app).setRotation((float)pitchQ1, (float)rollQ1, (float)headingQ1);
//                }
    			//do something
    			break;
    		case Sensor.TYPE_ROTATION_VECTOR:
                System.out.println("TYPE_ROTATION_VECTOR");
...
//            }
    ...
    protected Sensor initSingleSensor( int type, String name ){
		...
		if(newSensor != null){
			if(sensorManager.registerListener(sensorListener, newSensor, SensorManager.SENSOR_DELAY_NORMAL)) {
        ...
        System.out.println("SensorRegistered");
		return newSensor;
    }  
